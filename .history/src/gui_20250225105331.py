...
class NotarIAApp:
    ...
    def escanear_documento(self):
        try:
            output_path_front = "scanned_document_front.png"
            output_path_back = "scanned_document_back.png"
        
            print("📄 Iniciando escaneo automático...")
            scan_document(output_path_front, output_path_back)

            # Cargar imágenes en la interfaz
            self.mostrar_imagen(output_path_front)
            self.imagen_front = output_path_front
            self.imagen_back = output_path_back

            self.mostrar_mensaje("✅ Escaneo completado con éxito.", "green")

        except Exception as e:
            self.mostrar_mensaje(f"❌ Error al escanear el documento: {str(e)}", "red")
